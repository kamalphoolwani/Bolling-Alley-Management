...
//	}
	...
	public boolean firstScore(int i, int curScore[]) {
		...
		if (strikeballs == 2){
			...
			int x=cumulScores[bowlIndex][i/2];
			x += 10;
			if(curScore[i+1] != -1) {
				x += curScore[i+1] + cumulScores[bowlIndex][(i/2)-1];
				if (curScore[i+2] != -1){
					if( curScore[i+2] != -2){
						x += curScore[i+2];
					}
				...
				if ( i/2 > 0 ){
					x += curScore[i+2] + cumulScores[bowlIndex][(i/2)-1];
				} else {
					x += curScore[i+2];
				}
				if (curScore[i+3] != -1){
	...
	public void normalThrow(int i,int curScore[]) {
		int x=cumulScores[bowlIndex][i/2];
		if( i%2 == 0 && i < 18){
			if ( i/2 == 0 ) {
				...
				if(curScore[i] != -2){
					x += cumulScores[bowlIndex][i/2 - 1] + curScore[i];
				} else {
					x += cumulScores[bowlIndex][i/2 - 1];
				}	
			}
		} else if (i < 18){ 
			if(curScore[i] != -1 && i > 2){
	...
	private int getScore( Bowler Cur, int frame) {
		...
		for (int i = 0; i != current+2; i++){
			...
			if( i%2 == 1 && curScore[i - 1] + curScore[i] == 10 && i < current - 1 && i < 19){
				...
				boolean dobreak=firstScore(i,curScore);
				if(dobreak==true) {
					break;
				}
			}else { 
...
//	}
	...
	public void publish( Lane event ) {
